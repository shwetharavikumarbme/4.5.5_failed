...
const AllPosts = ({ scrollRef, videoRefs, isTabActive }) => {
  ...
  const HEADER_HEIGHT = 60; // Adjust based on your actual header height
  const [headerHeight, setHeaderHeight] = useState(HEADER_HEIGHT);
  const scrollY = useRef(new Animated.Value(0)).current;
  const headerTranslateY = useRef(new Animated.Value(0)).current;
  const lastScrollY = useRef(0);

  const hideThreshold = 50; // Customize this based on UX
  const headerVisible = useRef(true); // To avoid repeated animations


  // Replace your header animation logic with this
  const handleScroll = Animated.event(
    [{ nativeEvent: { contentOffset: { y: scrollY } } }],
    {
      ...
      listener: (event) => {
        ...
        // Only trigger hide/show if scroll is significant
        if (Math.abs(diff) > 5) {
          if (diff > 0 && currentY > hideThreshold && headerVisible.current) {
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: 'whitesmoke', }}>
        ...
        <Animated.View
          style={{
            ...
            left: 0,
            right: 0,
            height: headerHeight, // Explicit height
        ...
        <View style={styles.container}>
          ...
          {!loading ? (
            <AnimatedFlatList
              ...
              contentContainerStyle={{ flexGrow: 1, paddingTop: headerHeight, // Add padding equal to header height
                paddingBottom: 100 }}
              ListHeaderComponent={
                <>
